...
<?php
if ($result['usr_cli'] == $result['id_usr']){
  ?>
      <div class="row"  style="border:1px dashed #000;padding:10px;background-color:#ddd;">
          <div class="col-md-2" style="text-align:right;">
            ...
            <a href="usr-relatorio-conteudo.php?idusr=<?php echo $result['id_usr'] ?>&nomeusr=<?php echo $result['nome_usr'] ?>&idcli=<?php echo $result['id_cli'] ?>&imglogo=<?php echo $result['imglogo_cli'] ?>&empresa=<?php echo $result['empresa_cli'] ?>" class="btn btn-info" style="width:150px;margin:2px;" role="button">CONTEÚDO</a>              
          </div>        
          <div class="col-md-10" style="text-align:left;">
            ...
            <strong>EMPRESA:</strong> <? echo $result['empresa_cli'] ?><br />            
<br />
      ...
      <?php  
       ...
       if ($total > 0):
        ...
        // SQL para selecionar os registros
        $sql = "SELECT * FROM tbl_usr inner join tbl_cli on tbl_usr.id_cli = tbl_cli.id_cli WHERE tbl_cli.usr_cli != tbl_usr.id_usr";
        $stmt = $PDO->prepare($sql);
        $stmt->execute();
       while ($user = $stmt->fetch(PDO::FETCH_ASSOC)):  
        //if ($user['id_usr'] == $user['usr_cli']){
      ?>
