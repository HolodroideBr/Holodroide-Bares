...
/// added by me
		...
		if (@file_exists($saveToFile) && $Resume['use'] === TRUE) {
            ...
            echo    $saveToFile = dirname($saveToFile) . PATH_SPLITTER . $options['download_dir'].time() . "_" . basename($saveToFile);
            }
			 if (@!file_exists($saveToFile)) {
		...
		// here we test extensions
              if (!in_array(end(explode(".", strtolower($ext))), $allowedExtensions))
                  {
...
$youtube_name = $original_name;
		$original_name=str_replace(array("_","-"), " ", $original_name);
		$original_name=str_replace(array("(",""), " ", $original_name);
		$original_name=str_replace(array(")",""), " ", $original_name);
            if (stristr($original_name, 'YT')) {
            
          
          $video_id = get_last_url($youtube_name);
           $youtube_details = get_youtube_id($video_id, $apikey);
          
      
		   $image =   $youtube_details[0];
		  $duration = $youtube_details[1];
		  echo " <br><p><img  src=\"$image\"></p>";
        if($duration > $max_youtube_duration ){
            
           
          echo lang(391);
          echo lang(392);
          echo "<strong>".$max_youtube_duration." minutes</strong>";
           exit;   
        }
